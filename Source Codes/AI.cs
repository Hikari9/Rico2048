...
namespace _2048
{
    public partial class AI : Form
    {
        ...
        /*
        public int eval(int[,] gr = null){
            if( gr == null ) gr = g;
            return 0;
        }

        int[] search(int depth, int alpha, int beta, int positions, int cutoffs, bool player, int[,] gr = null)
        {
            if (gr == null) gr = g;
            int bestScore;
            int bestMove = -1;
            ...
            if (player)
            {
                ...
                for (int d = 0; d < 4; ++d)
                {
                    int[,] newGrid = CloneGrid(gr);
                    if (Slide((MOVE)d, newGrid))
                    {
                        positions++;
                        if (IsWin(newGrid))
                        {
                            result = new int[] { 0, 10000, positions, cutoffs };
                        }
                    }
                    // make AI
                    int[,] AI = CloneGrid(newGrid);
                    if (depth == 0)
                    {
                        result = new int[] { d, eval(AI) };
                    }
                    else
                    {
                        result = search(depth - 1, bestScore, beta, positions, cutoffs, true, AI);
                        if (result[1] > 9900)
                        {
                            result[1]--;
                        }
                    ...
                    {
                        cutoffs++;
                        return new int[] { bestMove, beta, positions, cutoffs };
                    }
                }
